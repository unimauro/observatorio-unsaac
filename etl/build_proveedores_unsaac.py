...
CODIGO='001909'; RUC_ENT='20172474501'
def tp(ruc):
    p=str(ruc)[:2]
...
out={
 '_meta':{
  ...
  'entidad':'Universidad Nacional de San Antonio Abad del Cusco (UNSAAC) - pliego 511',
  'ruc':RUC_ENT,
  'codigoentidad_conosce':int(CODIGO),
  'periodo':'2023-2025',
  'extraido':'2026-07',
  'unidad_monto':'Soles (PEN), monto adjudicado por item',
  'nota':"Agregado desde reportes anuales CONOSCE de Adjudicaciones (nivel item de buena pro), archivos CONOSCE_ADJUDICACIONES{2023,2024,2025}_0.xlsx, filtrando codigoentidad 001909 / RUC 20172474501 (UNIVERSIDAD NACIONAL DE SAN ANTONIO ABAD DEL CUSCO - UNSAAC, entidad de Gobierno Nacional; NO confundir con el Gobierno Regional de Cusco, municipalidades del Cusco ni la Universidad Nacional Diego Quispe Tito). 'monto' = suma de monto_adjudicado_item_soles. 'n' = numero de procesos (codigoconvocatoria) distintos en que ese proveedor obtuvo buena pro. 'tipos' = procesos por tipo de proceso de seleccion. 'objeto' = items por objeto contractual (Bien/Servicio/Obra). tipo_persona por prefijo de RUC (20=empresa, 10/15=natural); consorcios y no domiciliados clasificados como empresa. NO incluye ordenes de compra <8 UIT (dataset aparte)."
 },
 'totales':{
...
json.dump(out, open(os.path.join(HERE,'..','data','proveedores-unsaac.json'),'w'),
